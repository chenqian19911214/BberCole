...
import com.bber.company.android.util.MPermissionUtils;
import com.bber.company.android.util.OtherUtils;
import com.bber.company.android.util.PhotoUtils;
import com.bber.company.android.view.adapter.FolderAdapter;
import com.bber.company.android.view.adapter.PhotoAdapter;
...
public class PhotoPickerActivity extends AppCompatActivity implements PhotoAdapter.PhotoClickCallBack {
    ...
    private List<PhotoBean> mPhotoLists = new ArrayList<>();
...
//        });
    ...
    private void initIntentParams() {
        mIsShowCamera = getIntent().getBooleanExtra(ChatPicViewModel.EXTRA_SHOW_CAMERA, false);
        mSelectMode = getIntent().getIntExtra(ChatPicViewModel.EXTRA_SELECT_MODE, ChatPicViewModel.MODE_SINGLE);
        mMaxNum = getIntent().getIntExtra(ChatPicViewModel.EXTRA_MAX_MUN, ChatPicViewModel.DEFAULT_NUM);
        mTotalNum = getIntent().getIntExtra(ChatPicViewModel.TOTAL_MAX_MUN, 0);
...
//        }
    ...
    private void getPhotosSuccess() {
        ...
        final List<PhotoFolderBean> folders = new ArrayList<>();
        for (String key : keys) {
            if (ALL_PHOTO.equals(key)) {
                PhotoFolderBean folder = mFolderMap.get(key);
    ...
    private void selectPhoto(PhotoBean photo) {
        ...
        if (mSelectMode == ChatPicViewModel.MODE_SINGLE) {
     ...
     * @param folders
     */
    private void toggleFolderList(final List<PhotoFolderBean> folders) {
        //初始化文件夹列表
        if (!mIsFolderViewInit) {
            ViewStub folderStub = findViewById(R.id.floder_stub);
            folderStub.inflate();
            View dimLayout = findViewById(R.id.dim_layout);
            mFolderListView = findViewById(R.id.listview_floder);
            ...
            mFolderListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    for (PhotoFolderBean folder : folders) {
                        folder.setIsSelected(false);
                    }
                    PhotoFolderBean folder = folders.get(position);
                    folder.setIsSelected(true);
     ...
     * @param photoFolder
     */
    public void selectFolder(PhotoFolderBean photoFolder) {
